...
static file_data_ctx_t file_info_table[TABLE_SIZE];
  ...
  while ((ent = readdir(dir)) != NULL) {
    ...
    if ((match == NULL) || (fnmatch(match, tpath, (FNM_PERIOD)) == 0)) {
      ...
      if (copy_flag) {
        ...
        // new
        if (nfiles) {
          memcpy(file_info_table[nfiles - 1].file_name, ent->d_name, sizeof(ent->d_name));
          memcpy(file_info_table[nfiles - 1].file_size, size, sizeof(size));
        }
      }
    }
  }
...
int fm_file_table_create(char ***list_out, uint16_t *files_number, const char *filter_suffix) {
  ...
  ESP_LOGI(TAG, "g_root_path = %s", g_root_path);
  ...
  ESP_LOGI(TAG, "file number = %d", f_num);
  rewinddir(p_dir);
...
void read_file_info(file_data_ctx_t *file_data) {
  ...
  file_data->nfiles = file_info.nfiles;
  return;
}

void read_file_info_table(file_data_ctx_t **tb_p) {
  *tb_p = file_info_table;
}
