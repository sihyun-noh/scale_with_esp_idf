...
static const console_cmd_plugin_desc_t
    __attribute__((section(".console_cmd_desc"), used)) PLUGIN = { .name = "console_cmd_mqtt",
...
static const char *mqtt_state_to_string(mqtt_client_state_t state) {
  switch (state) {
    case MQTT_STATE_INIT: return "Initializing";
    ...
    case MQTT_STATE_STOPPED: return "Disconnected and Stopped";
...
esp_err_t set_mqtt_client_cert(const uint8_t *client_cert_pem_start_i, const uint8_t *client_cert_pem_end_i) {
  if (!client_cert_pem_start_i || !client_cert_pem_end_i || (client_cert_pem_start_i > client_cert_pem_end_i)) {
...
esp_err_t set_mqtt_client_key(const uint8_t *client_key_pem_start_i, const uint8_t *client_key_pem_end_i) {
  if (client_key_pem_start_i && client_key_pem_end_i && (client_key_pem_start_i >= client_key_pem_end_i)) {
...
esp_err_t set_mqtt_broker_certs(const uint8_t *ca_cert_pem_start_i, const uint8_t *ca_cert_pem_end_i) {
  if (!ca_cert_pem_start_i || !ca_cert_pem_end_i || (ca_cert_pem_start_i > ca_cert_pem_end_i)) {
...
static int do_mqtt_pub_cmd(int argc, char **argv) {
  ...
  msg_id =
      esp_mqtt_client_publish(client_handle, mqtt_pub_args.topic->sval[0], mqtt_pub_args.message->sval[0], 0, 1, 0);
  if (msg_id == -1) {
    ESP_LOGE(TAG, "mqtt client not connected");
    return 0;
  }
  ESP_LOGI(TAG, "Publish successful, msg_id=%d, topic=%s, data=%s", msg_id, mqtt_pub_args.topic->sval[0],
...
esp_err_t console_cmd_mqtt_register(void) {
  ...
  mqtt_args.username =
      arg_str0("u", "username", "<username>", "Provide a username to be used for authenticating with the broker");
  mqtt_args.password =
      arg_str0("P", "password", "<password>", "Provide a password to be used for authenticating with the broker");
  mqtt_args.cert =
      arg_lit0(NULL, "cert",
               "Define the PEM encoded certificate for this client, if required by the broker (flag, no argument)");
  mqtt_args.key = arg_lit0(
      NULL, "key", "Define the PEM encoded private key for this client, if required by the broker (flag, no argument)");
  mqtt_args.cafile =
      arg_lit0(NULL, "cafile", "Define the PEM encoded CA certificates that are trusted (flag, no argument)");
#if defined(CONFIG_MBEDTLS_CERTIFICATE_BUNDLE)
  mqtt_args.use_internal_bundle =
      arg_lit0(NULL, "use-internal-bundle", "Use the internal certificate bundle for TLS (flag, no argument)");
#endif
  ...
  const esp_console_cmd_t mqtt_cmd = {
    .command = "mqtt", .help = "mqtt command", .hint = NULL, .func = &do_mqtt_cmd, .argtable = &mqtt_args
  };
  ...
  const esp_console_cmd_t mqtt_pub_cmd = { .command = "mqtt_pub",
  ...
  const esp_console_cmd_t mqtt_sub_cmd = { .command = "mqtt_sub",
