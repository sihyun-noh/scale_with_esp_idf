...
class ESP32 {
  ...
  uint32_t getFreeHeap(void) { return heap_caps_get_free_size(MALLOC_CAP_INTERNAL); }

  uint32_t getMinFreeHeap() { return heap_caps_get_minimum_free_size(MALLOC_CAP_INTERNAL); }
...
AsyncMqttClient::AsyncMqttClient()
  ...
  // _client.onError([](void* obj, AsyncClient* c, int8_t error) {
  // (static_cast<AsyncMqttClient*>(obj))->_onError(error); }, this); _client.onTimeout([](void* obj, AsyncClient* c,
  // uint32_t time) { (static_cast<AsyncMqttClient*>(obj))->_onTimeout(); }, this);
  _client.onAck(
      [](void* obj, AsyncClient* c, size_t len, uint32_t time) { (static_cast<AsyncMqttClient*>(obj))->_onAck(len); },
      this);
  _client.onData([](void* obj, AsyncClient* c, void* data,
                    size_t len) { (static_cast<AsyncMqttClient*>(obj))->_onData(static_cast<char*>(data), len); },
                 this);
...
void AsyncMqttClient::_onConnect() {
  LOGI(TAG, "TCP conn, MQTT CONNECT");
#if ASYNC_TCP_SSL_ENABLED
  ...
  AsyncMqttClientInternals::OutPacket* msg =
      new AsyncMqttClientInternals::ConnectOutPacket(_cleanSession, _username, _password, _willTopic, _willRetain,
  ...
  do {
    switch (_parsingInformation.bufferState) {
      case AsyncMqttClientInternals::BufferState::NONE:
        ...
        switch (_parsingInformation.packetType) {
          ...
          case AsyncMqttClientInternals::PacketType.PINGRESP:
            LOGI(TAG, "rcv PINGRESP");
            _currentParsedPacket = new AsyncMqttClientInternals::PingRespPacket(
                &_parsingInformation, std::bind(&AsyncMqttClient::_onPingResp, this));
            break;
          ...
          case AsyncMqttClientInternals::PacketType.UNSUBACK:
            LOGI(TAG, "rcv UNSUBACK");
            _currentParsedPacket = new AsyncMqttClientInternals::UnsubAckPacket(
                &_parsingInformation, std::bind(&AsyncMqttClient::_onUnsubAck, this, std::placeholders::_1));
            break;
          case AsyncMqttClientInternals::PacketType.PUBLISH:
            LOGI(TAG, "rcv PUBLISH");
            _currentParsedPacket = new AsyncMqttClientInternals::PublishPacket(
                &_parsingInformation,
                std::bind(&AsyncMqttClient::_onMessage, this, std::placeholders::_1, std::placeholders::_2,
          ...
          case AsyncMqttClientInternals::PacketType.PUBREL:
            LOGI(TAG, "rcv PUBREL");
            _currentParsedPacket = new AsyncMqttClientInternals::PubRelPacket(
                &_parsingInformation, std::bind(&AsyncMqttClient::_onPubRel, this, std::placeholders::_1));
            break;
          case AsyncMqttClientInternals::PacketType.PUBACK:
            LOGI(TAG, "rcv PUBACK");
            _currentParsedPacket = new AsyncMqttClientInternals::PubAckPacket(
                &_parsingInformation, std::bind(&AsyncMqttClient::_onPubAck, this, std::placeholders::_1));
            break;
          case AsyncMqttClientInternals::PacketType.PUBREC:
            LOGI(TAG, "rcv PUBREC");
            _currentParsedPacket = new AsyncMqttClientInternals::PubRecPacket(
                &_parsingInformation, std::bind(&AsyncMqttClient::_onPubRec, this, std::placeholders::_1));
            break;
          case AsyncMqttClientInternals::PacketType.PUBCOMP:
            LOGI(TAG, "rcv PUBCOMP");
            _currentParsedPacket = new AsyncMqttClientInternals::PubCompPacket(
                &_parsingInformation, std::bind(&AsyncMqttClient::_onPubComp, this, std::placeholders::_1));
            break;
          default:
            LOGI(TAG, "rcv PROTOCOL VIOLATION");
            disconnect(true);
            break;
        }
        break;
      case AsyncMqttClientInternals::BufferState::REMAINING_LENGTH:
        ...
        if (currentByte >> 7 == 0) {
          _parsingInformation.remainingLength =
              AsyncMqttClientInternals::Helpers::decodeRemainingLength(_remainingLengthBuffer);
          _remainingLengthBufferPosition = 0;
      ...
      case AsyncMqttClientInternals::BufferState::PAYLOAD:
        ...
        break;
      default: currentBytePosition = len;
    }
  } while (currentBytePosition != len);
}

void AsyncMqttClient::_onPoll() {
  // if there is too much time the client has sent a ping request without a response, disconnect client to avoid half
  // open connections
  if (_lastPingRequestTime != 0 && (millis() - _lastPingRequestTime) >= (_keepAlive * 1000 * 2)) {
    LOGW(TAG, "PING t/o, disconnecting");
    disconnect(true);
    return;
  }
  ...
  if (_state == CONNECTED && _lastPingRequestTime == 0 &&
      (millis() - _lastClientActivity) >= (_keepAlive * 1000 * 0.7)) {
    _sendPing();
...
void AsyncMqttClient::_insert(AsyncMqttClientInternals::OutPacket* packet) {
  ...
  LOGI(TAG, "new insert #%u", packet->packetType());
...
void AsyncMqttClient::_addFront(AsyncMqttClientInternals::OutPacket* packet) {
  ...
  xSemaphoreTake(_xSemaphore, portMAX_DELAY);
  LOGI(TAG, "new front #%u", packet->packetType());
  if (_head == nullptr) {
    _tail = packet;
  } else {
    packet->next = _head;
...
void AsyncMqttClient::_addBack(AsyncMqttClientInternals::OutPacket* packet) {
  xSemaphoreTake(_xSemaphore, portMAX_DELAY);
  LOGI(TAG, "new back #%u", packet->packetType());
  if (!_tail) {
    _head = packet;
  } else {
    _tail->next = packet;
...
void AsyncMqttClient::_handleQueue() {
  ...
  while (_head && _client.space() > 10) {  // safe but arbitrary value, send at least 10 bytes
    ...
    if (_head->size() > _sent) {
      ...
      size_t realSent = _client.add(reinterpret_cast<const char*>(_head->data(_sent)), willSend,
                                    ASYNC_WRITE_FLAG_COPY);  // flag is set by LWIP anyway, added for clarity
      _sent += willSend;
      (void)realSent;
      _client.send();
      ...
      _lastPingRequestTime = 0;
#if ASYNC_TCP_SSL_ENABLED
      LOGI(TAG, "snd #%u: (tls: %u) %u/%u", _head->packetType(), realSent, _sent, _head->size());
#else
      LOGI(TAG, "snd #%u: %u/%u", _head->packetType(), _sent, _head->size());
#endif
    ...
    if (_head->size() == _sent) {
      if (_head->released()) {
        LOGI(TAG, "p #%d rel", _head->packetType());
  ...
  xSemaphoreGive(_xSemaphore);
  if (disconnect) {
    LOGI(TAG, "snd DISCONN, disconnecting");
    _client.close();
  }
}
  ...
  while (packet) {
    ...
    if (keepSessionData) {
      if (packet->qos() > 0 && packet->size() <= _sent) {  // check for qos includes check for PUB-packet type
        ...
        LOGI(TAG, "keep #%u", packet->packetType());
        xSemaphoreGive(_xSemaphore);
        _addBack(packet);
        xSemaphoreTake(_xSemaphore, portMAX_DELAY);
        packet = next;
...
uint16_t AsyncMqttClient::publish(const char* topic, uint8_t qos, bool retain, const char* payload, size_t length,
  ...
  AsyncMqttClientInternals::OutPacket* msg =
      new AsyncMqttClientInternals::PublishOutPacket(topic, qos, retain, payload, length);
  _addBack(msg);
  return msg->packetId();
}
