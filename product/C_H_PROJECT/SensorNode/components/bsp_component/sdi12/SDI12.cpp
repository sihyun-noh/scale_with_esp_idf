...
sdi12timer_t SDI12Timer::SDI12TimerRead(void) {
  ...
  // 6
  return ((sdi12timer_t)esp_timer_get_time() >> 6);
}
...
void pinMode_esp(gpio_num_t pin, esp_gpio_mode_t mode) {
  ...
  switch (mode) {
    case ESP_INPUT: io_conf.mode = GPIO_MODE_INPUT; break;
    case ESP_OUTPUT: io_conf.mode = GPIO_MODE_OUTPUT; break;
    case ESP_INPUT_PULLUP:
      io_conf.mode = GPIO_MODE_INPUT;
      io_conf.pull_up_en = GPIO_PULLUP_ENABLE;
      break;
    case ESP_INPUT_PULLDOWN:
      io_conf.mode = GPIO_MODE_INPUT;
...
void IRAM_ATTR SDI12::isr_wrapper(void* arg) {
  if (arg) {
    static_cast<SDI12*>(arg)->handleInterrupt_idf();  // 비정적 멤버 함수 호출
  }
}

void SDI12::handleInterrupt_idf() {
  receiveISR();  // 기존 내부 인터럽트 처리 로직
}
...
void SDI12::wakeSensors(int8_t extraWakeTime) {
  ...
  // timings for break and from the recorder are not critical.
  // Interrupts on the pin are disabled for the entire transmitting state
  gpio_set_level((gpio_num_t)_dataPinTX, 1);  // break is HIGH
  esp_rom_delay_us(lineBreak_micros);         // Required break of 12 milliseconds (12,000 µs)
  vTaskDelay(pdMS_TO_TICKS(extraWakeTime));
  gpio_set_level((gpio_num_t)_dataPinTX, 0);  // marking is LOW
  esp_rom_delay_us(marking_micros);           // Required marking of 8.33 milliseconds(8,333 µs)
}
...
void SDI12::writeChar(uint8_t outChar) {
  ...
  // _ALL_ interrupts disabled so timing can't be shifted
  ...
  gpio_set_level((gpio_num_t)_dataPinTX, 1);  // Start bit (inverse logic: HIGH)
  ...
  while (currentTxBitNum++ < lastHighBit) {
    ...
    if (bitValue) {
      gpio_set_level((gpio_num_t)_dataPinTX, 0);  // set the pin state to LOW for 1's

    } else {
      gpio_set_level((gpio_num_t)_dataPinTX, 1);  // set the pin state to HIGH for 0's
    }
...
void SDI12::sendResponse(const char* resp) {
  setState(SDI12_TRANSMITTING);               // Get ready to send data to the recorder
                                              //  digitalWrite(_dataPin, LOW);        // marking is LOW
  gpio_set_level((gpio_num_t)_dataPinRX, 1);  // marking is LOW
  esp_rom_delay_us(marking_micros);           // 8.33 ms marking before response
...
void SDI12::receiveISR() {
  ...
  uint8_t pinLevel = gpio_get_level((gpio_num_t)_dataPinRX);  // current RX data level
