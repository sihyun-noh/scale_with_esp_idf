...
void WifiStation::HandleScanResult() {
    ...
    for (int i = 0; i < ap_num; i++) {
        ...
        if (it != ssid_list.end()) {
            ESP_LOGI(TAG, "Found AP_NOH: %s, BSSID: %02x:%02x:%02x:%02x:%02x:%02x, RSSI: %d, Channel: %d, Authmode: %d",
                     (char*)ap_record.ssid, ap_record.bssid[0], ap_record.bssid[1], ap_record.bssid[2],
                     ap_record.bssid[3], ap_record.bssid[4], ap_record.bssid[5], ap_record.rssi, ap_record.primary,
                     ap_record.authmode);
