...
int read_soil_ec(void) {
    ...
    for (int i = 0; i < num_characteristic; i++) {
        if (mb_master_read_characteristic(mb_characteristic[i].cid, mb_characteristic[i].name, value, &data_len) ==
            -1) {
            ...
            if (is_battery_model()) {
                ...
                LOGI(TAG, "SEN1 :Power mode > ec : %.2f, moisture : %.2f, temperature : %.2f", f_saturation_ec, f_mos,
                     f_temp);
            }
