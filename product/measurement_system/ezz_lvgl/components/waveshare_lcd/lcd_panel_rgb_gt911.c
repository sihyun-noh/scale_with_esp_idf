...
static void example_lvgl_flush_cb(lv_display_t *display, const lv_area_t *area, void *px_map) {
  esp_lcd_panel_handle_t panel_handle = (esp_lcd_panel_handle_t)lv_display_get_driver_data(display);
...
void example_touchpad_read(lv_indev_t *indev, lv_indev_data_t *data) {
  ...
  bool touchpad_pressed =
      esp_lcd_touch_get_coordinates(lv_indev_get_user_data(indev), touchpad_x, touchpad_y, NULL, &touchpad_cnt, 1);
...
void lv_port_12c_tp_init() {
  ...
  ESP_LOGI(TAG, "Read & write test to touch controller GT911");
  ...
  write_buf = 0x2E;  // 또는 0x0E(CAN 안 쓸 때)
...
void lv_port_disp_init(void) {
  ...
  // LVGL 9 uses draw buffers configured directly on the display
#else
...
#if CONFIG_EXAMPLE_DOUBLE_FB
  lv_display_set_buffers(disp, buf1, buf2, EXAMPLE_LCD_H_RES * EXAMPLE_LCD_V_RES * sizeof(lv_color_t),
                         LV_DISPLAY_RENDER_MODE_PARTIAL);
#else
  lv_display_set_buffers(disp, buf1, buf2, EXAMPLE_LCD_H_RES * 100 * sizeof(lv_color_t),
                         LV_DISPLAY_RENDER_MODE_PARTIAL);
#endif
}
