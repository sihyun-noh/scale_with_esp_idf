...
extern void wifi_connect_helper(void);
extern void create_thingsboard_task(void);
...
void app_main(void) {
    ...
    EventManager::GetInstance().GetEventGroup(EVENT_GROUPS_FOR_THINGSBOARD);
    create_thingsboard_task();
    //    battery_loop_task();

    TaskManager& mgr = TaskManager::getInstance();

    // SUBJECT Tasks 생성 (각각 SENSOR1, SENSOR2)
    mgr.createAndStartTask(TaskType::SUBJECT, SubjectType::SENSOR1, "SubjectTask1", 4096, 5);
    mgr.createAndStartTask(TaskType::SUBJECT, SubjectType::SENSOR2, "SubjectTask2", 4096, 5);

    // Observer Tasks 생성
    // Controller: SENSOR1만 구독
    std::vector<SubjectType> controllerSubs = { SubjectType::SENSOR1 };
    mgr.createAndStartTask(TaskType::CONTROLLER, "ControllerTask", 4096, 5, controllerSubs);

    // Display: SENSOR2만 구독
    std::vector<SubjectType> displaySubs = { SubjectType::SENSOR2 };
    mgr.createAndStartTask(TaskType::DISPLAY, "DisplayTask", 4096, 5, displaySubs);

    // Logging: SENSOR2만 구독 (예시)
    // std::vector<SubjectType> loggingSubs = { SubjectType::SENSOR2 };
    // mgr.createAndStartTask(TaskType::LOGGING, "LoggingTask", 4096, 5, loggingSubs);

    // Network: SENSOR1과 SENSOR2 모두 구독
    std::vector<SubjectType> networkSubs = { SubjectType::SENSOR1, SubjectType::SENSOR2 };
    mgr.createAndStartTask(TaskType::NETWORK, "NetworkTask", 4096, 5, networkSubs);

    // Global Observer Task 생성: 모든 Subject의 큐를 폴링하여 통합 처리
    mgr.createAndStartTask(TaskType::GLOBAL_OBSERVER, "GlobalObserverTask", 4096, 5);

    // 특정 한나의 task에만 메시지 발행
    ITask* targetTask = mgr.getTaskByType(TaskType::CONTROLLER);

    // 여러 task에게 메시지 발행

    std::vector<ITask*> targets;

    std::vector<TaskType> targetTypes = {
        //        TaskType::CONTROLLER,
        TaskType::DISPLAY,
        TaskType::NETWORK,
    };

    for (TaskType type : targetTypes) {
        auto tasks = mgr.getTasksByType(type);
        targets.insert(targets.end(), tasks.begin(), tasks.end());
    }

    while (true) {
        // battery_loop_task();
        //  예시: 2초마다 메시지 999 발행 (모든 구독 Task가 handleMessage()를 통해 처리)

        if (targetTask != nullptr) {
            mgr.sendGlobalMessage(999, targetTask);
        }

        mgr.sendGlobalMessageToTargets(100, targets);

        // 특정 Observer에게 메시지를 보내려면, 미리 해당 Task의 포인터를 저장하여 전달하면 됩니다.
        LOGE(TAG, "Message send to ControllerTask");
