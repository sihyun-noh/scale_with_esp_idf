...
int init_littlefs_impl(const char *partition_name, const char *root_path) {
  ...
  if (ret != ESP_OK) {
    if (ret == ESP_FAIL) {
      ...
      ESP_LOGE(TAG, "Failed to find LittleFS parition");
    } else {
...
int format_littlefs_impl(void) {
  ...
  if (ret == ESP_FAIL) {
    ESP_LOGE(TAG, "Failed to format littlefs system");
    return -1;
  }
  return 0;
}
...
int show_file_littlefs_impl(void) {
  ...
  ESP_LOGI(TAG, "base dir = %s", s_root_path);
  ...
  if (d) {
    ...
    ESP_LOGI(TAG, "Failed to show file_log name");
    return -1;
  }
}
