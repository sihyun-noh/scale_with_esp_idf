...
static void esp_dump_per_task_heap_info(void) {
  ...
  heap_info.blocks =
      s_block_arr;  // Gets block wise allocation details. For each block, gets owner task, address and size
...
void HeapTaskTracking_task(void *args) {
  uint32_t size = 0;
  const char *TAG = "HeapTaskTracking_task";
  while (1) {
    /*
