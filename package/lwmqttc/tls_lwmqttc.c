...
lwmqtt_err_t tls_lwmqtt_network_connect(tls_lwmqtt_network_t *n, char *host, char *port) {
  ...
  if (ret != 0) {
    tls_log("mbedtls_ctr_drbg_seed", ret);
  ...
  if (n->ca_buf) {
    ...
    if (ret != 0) {
      tls_log("mbedtls_x509_crt_parse", ret);
      return LWMQTT_NETWORK_FAILED_CONNECT;
    }
  }
  ...
  if (ret != 0) {
    tls_log("mbedtls_net_connect", ret);
    return LWMQTT_NETWORK_FAILED_CONNECT;
  }
  ...
  if (ret != 0) {
    tls_log("mbedtls_ssl_config_defaults", ret);
    return LWMQTT_NETWORK_FAILED_CONNECT;
  }
  ...
  if (ret != 0) {
    tls_log("mbedtls_ssl_set_hostname", ret);
    return LWMQTT_NETWORK_FAILED_CONNECT;
  }
  ...
  if (ret != 0) {
    tls_log("mbedtls_ssl_handshake", ret);
    return LWMQTT_NETWORK_FAILED_CONNECT;
  }
...
lwmqtt_err_t tls_lwmqtt_network_wait(tls_lwmqtt_network_t *n, bool *connected, uint32_t timeout) {
  ...
  *connected = result > 0;
  ...
  if (ret < 0) {
    tls_log("mbedtls_net_set_block", ret);
    return LWMQTT_NETWORK_FAILED_CONNECT;
  }
...
lwmqtt_err_t tls_lwmqtt_network_peek(tls_lwmqtt_network_t *n, size_t *available, uint32_t timeout) {
  ...
  if (ret != 0) {
    tls_log("mbedtls_net_set_nonblock", ret);
    return LWMQTT_NETWORK_FAILED_READ;
  }
...
lwmqtt_err_t tls_lwmqtt_network_read(void *ref, uint8_t *buffer, size_t len, size_t *received, uint32_t timeout) {
  // cast network reference
  tls_lwmqtt_network_t *n = (tls_lwmqtt_network_t *)ref;
...
lwmqtt_err_t tls_lwmqtt_network_write(void *ref, uint8_t *buffer, size_t len, size_t *sent, uint32_t timeout) {
  // cast network reference
  tls_lwmqtt_network_t *n = (tls_lwmqtt_network_t *)ref;
