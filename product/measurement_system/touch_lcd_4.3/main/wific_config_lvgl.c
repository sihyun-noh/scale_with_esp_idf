...
static void wifi_timeout_timer_create(esp_timer_cb_t callback, const char *name, uint32_t time_ms) {
  esp_timer_create_args_t timer_conf = { .callback = callback, .name = name };
...
static void preload_create(void) {
  ...
  g_preload = lv_spinner_create(g_contain, 1000, 60);
  lv_obj_set_size(g_preload, 100, 100);
  lv_obj_align(g_preload, LV_ALIGN_CENTER, 0, 0);
}
...
static void wifi_connect_success(void) {
  ...
  ESP_LOGW(TAG, "connect success_contiune");
...
static void ap_page_click_connect_cb(lv_obj_t *obj, lv_event_t event) {
  if (LV_EVENT_CLICKED == event) {
    ...
    lv_obj_align(pwd_label, LV_ALIGN_IN_TOP_MID, 0, 0);

    /* Create the password box */
    pwd_ta = lv_textarea_create(g_pswd_page, NULL);
    lv_obj_align(pwd_ta, LV_ALIGN_IN_TOP_MID, 0, 22);
    ...
    lv_obj_align(kb, LV_ALIGN_IN_BOTTOM_MID, 0, 0);
...
static void click_wifi_list_cb(lv_obj_t *obj, lv_event_t event) {
  if (LV_EVENT_CLICKED == event) {
    ...
    lv_obj_align(g_ap_page, LV_ALIGN_IN_BOTTOM_MID, 0, -5);

    lv_obj_t *ssid_label = lv_label_create(g_ap_page, NULL);
    lv_obj_set_width(ssid_label, lv_obj_get_width(g_ap_page));
    lv_obj_align(ssid_label, LV_ALIGN_IN_TOP_LEFT, 0, 0);
    char info_str[128] = { 0 };
    sprintf(info_str, "SSID: %s\nRSSI: %d\nAuth: %s", wifi_config_data.ap_info_list[index].ssid,
    ...
    lv_obj_align(btn_cancel, LV_ALIGN_IN_BOTTOM_LEFT, 0, 0);
    label = lv_label_create(btn_cancel, NULL);
    lv_label_set_text(label, "Canael");
    ...
    lv_obj_align(btn_connect, LV_ALIGN_IN_BOTTOM_RIGHT, 0, 0);
    label = lv_label_create(btn_connect, NULL);
    lv_label_set_text(label, "Connect");
  }
}
...
void littlevgl_wificonfig_init(void) {
  ...
  lv_obj_set_size(g_wifi_list, lv_obj_get_width(g_contain),
                  lv_obj_get_height(g_contain) - lv_obj_get_height(g_wifi_config_label));
  lv_obj_align(g_wifi_list, LV_ALIGN_IN_TOP_LEFT, 0, lv_obj_get_height(g_wifi_config_label));
}
      ...
      case WIFI_EVENT_STA_DISCONNECTED: {
        ...
        switch (disconnected->reason) {
          case WIFI_REASON_ASSOC_TOOMANY:
            ESP_LOGW(
                TAG,
                "WIFI_REASON_ASSOC_TOOMANY Disassociated because AP is unable to handle all currently associated STAs");
...
static void initialise_wifi(void) {
  ...
  ESP_ERROR_CHECK(
      esp_event_handler_instance_register(WIFI_EVENT, ESP_EVENT_ANY_ID, &net_event_handler, NULL, &instance_any_id));
  ESP_ERROR_CHECK(
      esp_event_handler_instance_register(IP_EVENT, IP_EVENT_STA_GOT_IP, &net_event_handler, NULL, &instance_got_ip));
