  ...
  for (;;) {
    if (_get_async_event(&packet)) {
      ...
      if (esp_task_wdt_add(NULL) != ESP_OK) {
        LOGE(TAG, "Failed to add async task to WDT");
      }
#endif
      _handle_async_event(packet);
#if CONFIG_ASYNC_TCP_USE_WDT
      if (esp_task_wdt_delete(NULL) != ESP_OK) {
        LOGE(TAG, "Failed to remove loop task from WDT");
      }
#endif
    }
  }
...
bool AsyncClient::connect(IPAddress ip, uint16_t port) {
  if (_pcb) {
    LOGW(TAG, "already connected, state %d", _pcb->state);
    return false;
  }
  if (!_start_async_task()) {
    LOGE(TAG, "failed to start task");
    return false;
  }
...
bool AsyncClient::connect(const char *host, uint16_t port) {
  ...
  if (err == ERR_OK) {
    return connect(addr.u_addr.ip4.addr, port);
  } else if (err == ERR_INPROGRESS) {
    _connect_port = port;
    return true;
  }
  LOGE(TAG, "error: %d", err);
  return false;
}
...
int8_t AsyncClient::_lwip_fin(tcp_pcb *pcb, int8_t err) {
  if (!_pcb || pcb != _pcb) {
    LOGE(TAG, "0x%08x != 0x%08x", (uint32_t)pcb, (uint32_t)_pcb);
    return ERR_OK;
  }
  tcp_arg(_pcb, NULL);
  if (_pcb->state == LISTEN) {
...
int8_t AsyncClient::_poll(tcp_pcb *pcb) {
  ...
  if (_pcb_busy && _ack_timeout && (now - _pcb_sent_at) >= _ack_timeout) {
    _pcb_busy = false;
    LOGW(TAG, "ack timeout %d", pcb->state);
    if (_timeout_cb)
  ...
  if (_rx_since_timeout && (now - _rx_last_packet) >= (_rx_since_timeout * 1000)) {
    LOGW(TAG, "rx timeout %d", pcb->state);
    _close();
    return ERR_OK;
  }
  // Everything is fine
...
void AsyncClient::_dns_found(struct ip_addr *ipaddr) {
  if (ipaddr && ipaddr->u_addr.ip4.addr) {
    connect(ipaddr->u_addr.ip4.addr, _connect_port);
  } else {
    if (_error_cb) {
