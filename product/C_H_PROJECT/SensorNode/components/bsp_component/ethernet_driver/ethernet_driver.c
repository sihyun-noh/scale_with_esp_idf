...
static const char *TAG = "[ethDriver]";

#if CONFIG_BSP_SPI_ETHERNETS_NUM
#define SPI_ETHERNETS_NUM CONFIG_BSP_SPI_ETHERNETS_NUM
#else
#define SPI_ETHERNETS_NUM 0
#endif

#if CONFIG_BSP_USE_INTERNAL_ETHERNET
...
#define INIT_SPI_ETH_MODULE_CONFIG(eth_module_config, num)                          \
  do {                                                                              \
    eth_module_config[num].spi_cs_gpio = CONFIG_BSP_ETH_SPI_CS ##num## _GPIO;         \
    eth_module_config[num].int_gpio = CONFIG_BSP_ETH_SPI_INT ##num## _GPIO;           \
    eth_module_config[num].polling_ms = CONFIG_BSP_ETH_SPI_POLLING ##num## _MS;       \
    eth_module_config[num].phy_reset_gpio = CONFIG_BSP_ETH_SPI_PHY_RST ##num## _GPIO; \
    eth_module_config[num].phy_addr = CONFIG_BSP_ETH_SPI_PHY_ADDR ##num;             \
  } while (0)
...
static esp_eth_handle_t eth_init_internal(esp_eth_mac_t **mac_out, esp_eth_phy_t **phy_out) {
  ...
  esp32_emac_config.smi_mdc_gpio_num = CONFIG_BSP_ETH_MDC_GPIO;
  esp32_emac_config.smi_mdio_gpio_num = CONFIG_BSP_ETH_MDIO_GPIO;
#if CONFIG_BSP_USE_SPI_ETHERNET
...
#endif  // CONFIG_BSP_USE_SPI_ETHERNET
  // Create new ESP32 Ethernet MAC instance
  esp_eth_mac_t *mac = esp_eth_mac_new_esp32(&esp32_emac_config, &mac_config);
  // Create new PHY instance based on board configuration
#if CONFIG_BSP_ETH_PHY_IP101
  esp_eth_phy_t *phy = esp_eth_phy_new_ip101(&phy_config);
#elif CONFIG_BSP_ETH_PHY_RTL8201
  esp_eth_phy_t *phy = esp_eth_phy_new_rtl8201(&phy_config);
#elif CONFIG_BSP_ETH_PHY_LAN87XX
  esp_eth_phy_t *phy = esp_eth_phy_new_lan87xx(&phy_config);
#elif CONFIG_BSP_ETH_PHY_DP83848
  esp_eth_phy_t *phy = esp_eth_phy_new_dp83848(&phy_config);
#elif CONFIG_BSP_ETH_PHY_KSZ80XX
...
#endif  // CONFIG_BSP_USE_INTERNAL_ETHERNET
...
static esp_err_t spi_bus_init(void) {
  ...
  spi_bus_config_t buscfg = {
    .miso_io_num = CONFIG_BSP_ETH_SPI_MISO_GPIO,
    .mosi_io_num = CONFIG_BSP_ETH_SPI_MOSI_GPIO,
    .sclk_io_num = CONFIG_BSP_ETH_SPI_SCLK_GPIO,
    .quadwp_io_num = -1,
    .quadhd_io_num = -1,
  };
  ESP_GOTO_ON_ERROR(spi_bus_initialize(CONFIG_BSP_ETH_SPI_HOST, &buscfg, SPI_DMA_CH_AUTO), err, TAG,
                    "SPI host #%d init failed", CONFIG_BSP_ETH_SPI_HOST);
...
static esp_eth_handle_t eth_init_spi(spi_eth_module_config_t *spi_eth_module_config, esp_eth_mac_t **mac_out,
  ...
  spi_device_interface_config_t spi_devcfg = { .mode = 0,
                                               .clock_speed_hz = CONFIG_BSP_ETH_SPI_CLOCK_MHZ * 1000 * 1000,
                                               .queue_size = 20,
...
#if CONFIG_BSP_USE_KSZ8851SNL
  eth_ksz8851snl_config_t ksz8851snl_config = ETH_KSZ8851SNL_DEFAULT_CONFIG(CONFIG_BSP_ETH_SPI_HOST, &spi_devcfg);
...
#elif CONFIG_BSP_USE_DM9051
  eth_dm9051_config_t dm9051_config = ETH_DM9051_DEFAULT_CONFIG(CONFIG_BSP_ETH_SPI_HOST, &spi_devcfg);
...
#elif CONFIG_BSP_USE_W5500
  eth_w5500_config_t w5500_config = ETH_W5500_DEFAULT_CONFIG(CONFIG_BSP_ETH_SPI_HOST, &spi_devcfg);
  w5500_config.int_gpio_num = spi_eth_module_config->int_gpio;
  w5500_config.poll_period_ms = spi_eth_module_config->polling_ms;
  esp_eth_mac_t *mac = esp_eth_mac_new_w5500(&w5500_config, &mac_config);
  esp_eth_phy_t *phy = esp_eth_phy_new_w5500(&phy_config);
#endif  // CONFIG_BSP_USE_W5500
  // Init Ethernet driver to default and install it
  esp_eth_handle_t eth_handle = NULL;
  ...
  if (spi_eth_module_config->mac_addr != NULL) {
    ESP_GOTO_ON_FALSE(esp_eth_ioctl(eth_handle, ETH_CMD_S_MAC_ADDR, spi_eth_module_config->mac_addr) == ESP_OK, NULL,
                      err, TAG, "SPI Ethernet MAC address config failed");
  }
...
esp_err_t bsp_eth_init(esp_eth_handle_t *eth_handles_out[], uint8_t *eth_cnt_out) {
  esp_err_t ret = ESP_OK;
  esp_eth_handle_t *eth_handles = NULL;
  uint8_t eth_cnt = 0;

#if CONFIG_BSP_USE_INTERNAL_ETHERNET || CONFIG_BSP_USE_SPI_ETHERNET
  ESP_GOTO_ON_FALSE(eth_handles_out != NULL && eth_cnt_out != NULL, ESP_ERR_INVALID_ARG, err, TAG,
  ...
  spi_eth_module_config_t spi_eth_module_config[CONFIG_BSP_SPI_ETHERNETS_NUM] = { 0 };
...
#if CONFIG_BSP_SPI_ETHERNETS_NUM > 1
...
#endif  // CONFIG_BSP_USE_INTERNAL_ETHERNET || CONFIG_BSP_USE_SPI_ETHERNET
