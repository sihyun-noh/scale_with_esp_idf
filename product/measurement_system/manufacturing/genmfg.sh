...
echo "Generate MFG data for Weight Judge"
PROD_PATH=`pwd -P`
echo ${PROD_PATH}
...
selectDevice() {
  echo "Please select target chipset. 1: esp32, 2: esp32s2, 3: esp32s3"
  read DEVICE_ID
  if [ ${DEVICE_ID} = "1" ]; then
...
generateBin() {
  python3 ${TOOL_PATH}/esp-idf/tools/mass_mfg/mfg_gen.py generate weight_judge_config.csv weight_judge_master.csv weight_judge 0x6000
}
...
flashMFG() {
  esptool.py --chip ${IDF_TARGET} --port ${PORT_PATH} write_flash 0x844000 ./bin/weight_judge-1.bin
}
